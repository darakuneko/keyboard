...
void _set_code_to_name(const char * code_to_name, uint16_t keycode, uint8_t idx) {
  for (uint8_t i = idx; i < NAME_LENGH; i++) {
    if(i < CODE_NAME_LENGH + idx){
      name[i] = pgm_read_byte_near(code_to_name + keycode * CODE_NAME_LENGH + (i - idx));
    } else {
      name[i] = '\0';
    }
  }
...
void set_code_to_name(uint16_t keycode) {
    ...
    } else if(keycode >= 23746 && keycode <= 23765){
      _set_code_to_name(code_to_name23746, keycode - 23746, 0);
    }  else if(keycode >= 23906 && keycode <= 23908){
      _set_code_to_name(code_to_name23906, keycode - 23906, 0);
    } 
    ...
    else if(keycode >= R_HOLD_TAP_CTR_FIRST && keycode <= (long int)(RCTR_FIRST + LCTR_SHIFT_ALT_GUI_FIRST + NUM_USB_HID_KEYCODES)){
