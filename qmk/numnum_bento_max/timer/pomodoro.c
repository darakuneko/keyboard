...
static bool break_dismiss_notification = false;
...
static void pomodoro_next_work_interval(void) {
    break_dismiss_notification = false;
...
void pomodoro_update(void) {
    ...
    if (phase == POMODORO_BREAK || phase == POMODORO_LONG_BREAK) {
        
        // Auto dismiss notification after timeout or if double tap detected
        if (break_dismiss_notification || timer_elapsed32(break_notification_start_time) >= break_auto_dismiss_timeout || is_double_tap_detected) {
            break_dismiss_notification = true;
            return;
        }
