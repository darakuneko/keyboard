...
bool is_active_area(iqs5xx_data_t* const data) {  
...
static void handle_tap_area(iqs5xx_data_t* const data, bool is_active_area) {
  if(!is_active_area) return;
...
static void handle_single_tap(iqs5xx_data_t* const data, bool is_active_area) {
  if (!tapped && data->ges_evnet0 == 1) {
    ...
    } else if(is_active_area && (
      (!trackpad_config.drag_strength_mode && timer_elapsed32(timer.drag_time) > trackpad_config.drag_term) || 
...
void set_tap(iqs5xx_data_t* const data, report_mouse_t* const rep_mouse, bool is_active_area) {
  ...
  handle_tap_area(data, is_active_area);
  update_tap_counters(data);

  if(!is_trackpad_press){
    handle_single_tap(data, is_active_area);
...
static bool handle_scroll(iqs5xx_data_t* const data,
...
void set_gesture(iqs5xx_data_t* const data, report_mouse_t* const rep_mouse, bool is_active_area) {
  int32_t dx, dy;
  calculate_cursor_movement(data, &dx, &dy);

  if (data->finger_cnt == 0) {
    handle_short_scroll(data, rep_mouse);
  } else if (data->finger_cnt == 1 && is_active_area) {
    rep_mouse->x = dx;
    rep_mouse->y = dy;
  } else if (data->finger_cnt == 2) {
      bool is_scroll = handle_scroll(data, rep_mouse);
      if(!is_scroll) {
...
void set_trackpad_layer(iqs5xx_data_t* const data, bool is_active_area) {
  if (data->finger_cnt == 0) {
    ...
    if (!use_trackpad_layer && trackpad_config.can_trackpad_layer && is_active_area) {
...
void process_iqs5xx(iqs5xx_data_t* const data,
    ...
    bool _is_active_area = is_active_area(data);

    set_trackpad_layer(data, _is_active_area);
    set_tap(data, rep_mouse, _is_active_area);
    set_gesture(data, rep_mouse, _is_active_area);
}
